...
// GET all volunteer team members (Public)
router.get('/', async (req, res) => {
  try {
    const members = await VolunteerMember.find().sort({ createdAt: 1 });
    res.json(members);
  } catch (error) {
...
router.post('/', auth, upload.single('image'), async (req, res) => {
  try {
    ...
    const newMember = new VolunteerMember({
      name,
      role,
      image,
      imagePublicId
...
router.put('/:id', auth, upload.single('image'), async (req, res) => {
  try {
    ...
    if (!member) {
      if (req.file) fs.unlinkSync(req.file.path);
      return res.status(404).json({ error: 'Volunteer member not found.' });
    }
...
// DELETE volunteer team member (Protected)
router.delete('/:id', auth, async (req, res) => {
  try {
    const member = await VolunteerMember.findById(req.params.id);
    if (!member) return res.status(404).json({ error: 'Volunteer member not found.' });
    ...
    res.json({ message: 'Volunteer member deleted successfully.' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
